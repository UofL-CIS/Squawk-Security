...
namespace Squawk_Security.ClassLibrary.Services
{
    public class SerilogReportingService : IReportingService
    {
        ...
        public void SendAlert(string content, object details) =>
            _logger.Warning("[Alert] - {Content}", content, details.PrintPropertiesAsString());
        ...
        public void SendEvaluatedNetworkMessage(EvaluatedNetworkMessage evaluatedNetworkMessage) =>
            _logger.Information(
                ...
                evaluatedNetworkMessage.Capture.LinkLayerType, evaluatedNetworkMessage.GetCaptureChecksum(), evaluatedNetworkMessage.Capture.GetPacket().PrintPropertiesAsString());
    }
}
